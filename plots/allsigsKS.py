...
# soy is 'c', corn is 'orange', sorghum is 'm', wheat is (0.1, 0.7, 0.4) and wheat/soy is 'r'

#COLORLIST = [(0.1, 0.4, 0.1), (0.8, 0.5, 0)]  # poroto and pasture colors

COLORLIST = ['g', 'y', 'y']  # forest, nothing, and other
...
for k, signaturename in enumerate(signaturenames):
    ...
    for i, signature in enumerate(signatures.signatures):
        ...
        if signaturename[1].upper() in signature.name.upper():
            ...
            ax1.plot(signature.daysofyear, vivals,
                     color=color, linestyle=linestyles[j], label=lbl, linewidth=1.25, path_effects=pe)
