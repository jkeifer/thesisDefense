...
\begin{itemize}
  \item<1-> Better understanding of the dynamics of deforestation
  \item<2-> More effective land management policies
\end{itemize}
\end{frame}
...
Thus, NDVI 
\begin{itemize}
  \item<1-> is a ratioing index
  \item<2-> minimizes multiplicative noise
  \item<3-> has issues with non-linearity and additive noise
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item<2-> no saturate in high biomass
\end{itemize}
\end{frame}

\begin{frame}{Vegetation Indicies}
Need to be able to determine crop types
\begin{enumerate}
  \item<2-> What if two crops have similar VI values on a single date?
  \item<3-> How to determine the VI values of a crop in an image?
\end{enumerate}
\end{frame}
...
\begin{itemize}
  \item<2-> Use imagery from multiple dates to maximize the number of pixels that can be clearly classified.
\end{itemize}
\end{frame}

%Go into wardlow et al, MODIS sensor on the Terra and Aqua satellites
...
\begin{itemize}
  \item<2-> Traditional approaches all require training sites.
\end{itemize}
\vspace{\baselineskip}
\centering
